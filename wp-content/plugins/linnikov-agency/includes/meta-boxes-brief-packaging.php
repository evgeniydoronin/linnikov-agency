...
function linnikov_agency_register_brief_packaging_meta_boxes($post) {
  global $post;

  // Добавляем метабокс только для страницы с нужным шаблоном (например, 'Brief Branding')
  if ($post && get_page_template_slug($post->ID) === 'templates/page-brief-packaging.php') {
    linnikov_agency_add_meta_box(
      'linnikov_agency_slider_packaging_values',   // ID метабокса
      __('Slider Values', 'linnikov-agency'),   // Заголовок
      'linnikov_agency_slider_packaging_values_callback', // Колбэк-функция
      'page',   // Тип поста
...
add_action('add_meta_boxes', 'linnikov_agency_register_brief_packaging_meta_boxes');

// Колбэк-функция для метабокса с полями для ввода значений ползунка
function linnikov_agency_slider_packaging_values_callback($post) {
  // Получаем сохраненные данные
  $slider_values = get_post_meta($post->ID, '_linnikov_agency_slider_packaging_values', true);

  // Если нет данных, задаем значения по умолчанию
  if (!is_array($slider_values)) {
    $slider_values = [111, 222, 333, 444, 555]; // Значения по умолчанию
  }
...
function linnikov_agency_save_slider_packaging_values($post_id) {
  ...
  if (isset($_POST['linnikov_agency_slider_values']) && is_array($_POST['linnikov_agency_slider_values'])) {
    ...
    update_post_meta($post_id, '_linnikov_agency_slider_packaging_values', $slider_values);
  }
}
add_action('save_post', 'linnikov_agency_save_slider_packaging_values');